...
(function () {
  ...
  function ratingFor(g) { var h = hashStr(g.slug); return Math.min(4.9, Math.round((4.2 + (h % 80) / 100) * 10) / 10); }
  function playsFor(g) { var base = (g.popularWeight || 500) * 2317; return base >= 1e6 ? (base / 1e6).toFixed(1) + 'M' : Math.round(base / 1e3) + 'K'; }
  ...
  function cardHtml(g) {
    ...
    return '<a class="card' + (fav ? ' fav' : '') + '" href="' + detailUrl(g) + '" aria-label="' + escapeHtml(g.title) + '">' +
      ...
      '<div class="info">' +
        ...
        '<div class="meta"><span class="cat">' + escapeHtml(g.category) + '</span><span class="plays">' + playsFor(g) + ' plays</span></div>' +
      '</div>' +
    '</a>';
  }
