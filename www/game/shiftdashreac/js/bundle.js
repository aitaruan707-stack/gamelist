(function () {
  ...
  class RewardBtnScript extends Laya.Script {
      ...
      refreshSelf() {
          ...
          if (this.changeNodeName && !(t = e.seekChildByName(this.changeNodeName)))
              return void console.log('###没有对应名字的子节点');
          if (!this.shareSkinRes || !this.videoSkinRes)
              return void console.log('###无纹理资源');
  ...
  class se extends fx.BaseLogic {
      ...
      addReward_1(e, t = 1) {
          if (!e.num)
              return;
          let i = e.num * t;
          y.instance.getPlayerInfo().addCoins(i), console.log(`###addReward_1_激励广告 金币添加[${i}],111111`);
      }
      addReward_2(e, t = 1) {
          e.id &&
              (x.inst.isOwnSkin(e.id) ||
                  (x.inst.unlockSkin(e.id), console.log(`###addReward_2_激励广告 皮肤添加[${e.id}],222222`)));
      }
  }
  ...
  function callNativeMethod(methodName, parameters = []) {
      if (!(window.aitaBridge || (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.aitaBridge)))
          return Promise.resolve();
      const isAndroid = /Android/i.test(navigator.userAgent);
      return new Promise(function (resolve, reject) {
          let timestamp = new Date().getTime();
          let resultMethodName = methodName + timestamp + Math.random().toString(20).slice(2);
          window[resultMethodName] = function (result) {
              console.log("resultMethodName  delete  ", resultMethodName);
              delete window[resultMethodName];
              resolve(result);
          };
          parameters.push(resultMethodName);
          if (isAndroid) {
              window.aitaBridge[methodName].apply(window.aitaBridge, parameters);
          }
          else {
              window.webkit.messageHandlers.aitaBridge.postMessage({
  ...
  function callNativeMethod2(methodName, parameters = [], callback) {
      if (!(window.aitaBridge || (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.aitaBridge)))
          return Promise.resolve();
      const isAndroid = /Android/i.test(navigator.userAgent);
      let resultMethodName = methodName;
      window[resultMethodName] = function (result) {
  ...
  const AitaBridge = {
      inApp: !!(window.aitaBridge || (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.aitaBridge)),
      switchTab(index) {
          return callNativeMethod("switchTab", [index]);
      },
      getStatusBarHeight() {
          return callNativeMethod("getStatusBarHeight");
      },
      getUniqueID() {
          return callNativeMethod("getUniqueID");
      },
      getDtu() {
          return callNativeMethod("getDtu");
      },
      setClipboard(content) {
          return callNativeMethod("setClipboard", [content]);
      },
      getClipboard() {
          return callNativeMethod("getClipboard");
      },
      getAppCacheSize() {
          return callNativeMethod("getAppCacheSize");
      },
      clearAppCache() {
          return callNativeMethod("clearAppCache");
      },
      checkAppUpdate() {
          return callNativeMethod("checkAppUpdate");
      },
      setStorage(key, value) {
          return callNativeMethod("setStorage", [key, value]);
      },
      getStorage(key) {
          return callNativeMethod("getStorage", [key]);
      },
      openPage(url, title = "", showNav = false) {
          return callNativeMethod("openPage", [url, title, showNav]);
      },
      openPageScheme(url) {
          return callNativeMethod('launchScheme', [url]);
      },
      closePage() {
          return callNativeMethod("closePage");
      },
      wxShareFriend(params) {
          return callNativeMethod("wxShareFriend", [JSON.stringify(params)]);
      },
      wxShareTimeline(params) {
          return callNativeMethod("wxShareTimeline", [JSON.stringify(params)]);
      },
      steamAccount() {
          return callNativeMethod("steamAccount");
      },
      aliAuth() {
          return callNativeMethod("aliAuth");
      },
      aliPay(orderInfo) {
          return callNativeMethod("aliPay", [orderInfo]);
      },
      umOnPageStart(viewName) {
          return callNativeMethod("umOnPageStart", [viewName]);
      },
      umOnPageEnd(viewName) {
          return callNativeMethod("umOnPageEnd", [viewName]);
      },
      umOnEventObject(eventID, jsonStr) {
          return callNativeMethod("umOnEventObject", [eventID, jsonStr]);
      },
      sendBroadcast(methodName, parameters) {
          return callNativeMethod("sendBroadcast", [methodName, parameters]);
      },
      steamLogin() {
          return callNativeMethod("steamLogin");
      },
      wxShareMiniProgram(params) {
          return callNativeMethod("wxShareMiniProgram", [JSON.stringify(params)]);
      },
      clearWebViewCookies() {
          return callNativeMethod("clearWebViewCookies");
      },
      saveNetworkImage(url) {
          return callNativeMethod("saveNetworkImage", [url]);
      },
      getVersion() {
          return callNativeMethod("getVersion");
      },
      steamOrder(url, avatar, nickname, joinTime) {
          return callNativeMethod("steamOrder", [url, avatar, nickname, joinTime]);
      },
      toApplePay(id, price, name) {
          return callNativeMethod("toApplePay", [id, price, name]);
      },
      launchScheme(scheme) {
          return callNativeMethod("launchScheme", [scheme]);
      },
      wxPay(orderInfo) {
          return callNativeMethod("wxPay", [orderInfo]);
      },
      openPageTran(url, title = "", showNav = false) {
          return callNativeMethod("openPageTran", [url, title, showNav]);
      },
      showInterstitialAd(interstitialId) {
          return callNativeMethod("showInterstitialAd", [interstitialId]);
      },
      showRewardAd(rewardId) {
          return callNativeMethod("showRewardAd", [rewardId]);
      },
      openPageAd(url, title = "", showNav = false, showAd = true) {
          return callNativeMethod("openPageAd", [url, title, showNav, false]);
      },
      showRewardAdV2(rewardId, callback, customData) {
          let jsonStr = JSON.stringify({ rewardId: rewardId, customData: customData || '' });
          return callNativeMethod2("showRewardAd", [jsonStr], callback);
      },
  };
      ...
      onAdd() {
          ...
          (e.isBreakingIce = !0),
              ...
              this.on(fx.SdkEvent.E_REWARD_GAIN, this, t => {
                  console.log('TrySkinBreakingIceDialog11, 获取 Iron Man 皮肤碎片成功激励视频...AITA');
                  if (AitaBridge.inApp) {
                      fx.Utils.showTips("Ad is loading... Please wait.");
                      AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                          if (Number(res) === 1) {
                              if (fx.SdkCode.REWARD_GAIN_SUCCESS === t.code && this.btn_getVideoReward['$_GID'] === t.gid) {
                                  ...
                                  }
                                  {
                                      ...
                                      fx.Utils.showTips(`Got"${e}"Skin Fragment – Success!`);
                                  }
                                  let t = x.inst.getCfgById(this.skincfg.id).type;
                              ...
                              fx.Utils.showTips(`Reward claim failed.`);
                          }
                      }, '');
                  }
                  else {
                      ...
                      if (z.SKIN === e) {
                          ...
                          z.COINS === e &&
                              fx.Effect.getPropEffect(this.box_turnplate, this.box_Coins, Laya.Handler.create(this, () => {
                                  console.log('大转盘发放奖励：插屏广告', AitaBridge);
                                  if (AitaBridge.inApp) {
                                      AitaBridge.showInterstitialAd('ca-app-pub-8482820298032478/9335991971');
                                  }
                                  console.log('GameReviveDialog11, 复活,激励广告...AITA');
                                  if (AitaBridge.inApp) {
                                      fx.Utils.showTips("Ad is loading... Please wait.");
                                      AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                                          if (Number(res) === 1) {
                  ...
                  this.label_getBtn.visible &&
                      ...
                      ((this.label_getBtn['reg'] = !0),
                          this.label_getBtn.on(Laya.Event.CLICK, this, function (e) {
                              console.log('SignInDialog11, 普通签到成功1111...AITA');
                  ...
                  this.img_videoGetBtn.visible &&
                      ...
                      ((this.img_videoGetBtn['reg'] = !0),
                          this.on(fx.SdkEvent.E_REWARD_GAIN, this, function (e) {
                              ...
                              console.log('普通签到：插屏广告', AitaBridge);
                              if (AitaBridge.inApp) {
                                  AitaBridge.showInterstitialAd('ca-app-pub-8482820298032478/9335991971');
                              }
                          })));
  ...
  class TrySkinNumDialog extends ui.scenes.skin.TrySkinNumDialogUI {
      ...
      trySuccess() {
          console.log('获得一次皮肤碎片：插屏广告或者激励广告', AitaBridge);
          if (AitaBridge.inApp) {
              AitaBridge.showInterstitialAd('ca-app-pub-8482820298032478/9335991971');
          }
          let e = y.instance.getPlayerInfo();
          (e.trySkinNum = e.trySkinNum + 1),
              ...
              fx.Utils.showTips('Collected 1 Skin Fragment'),
              this.closeSelf();
      }
      closeSelf() {
          (this.box_scene3d.visible = !1),
  ...
  class SkinDialog extends ui.scenes.skin.SkinDialogUI {
                  ...
                  if (fx.SdkCode.REWARD_GAIN_SUCCESS === e.code)
                      if (this.btn_getcoins['$_GID'] === e.gid) {
                          if (AitaBridge.inApp) {
                              ...
                              AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                                  if (Number(res) === 1) {
                                      ...
                                      fx.Utils.showTips(`+ ${this.getVideoReward()} Coins`);
                                  }
                                  else {
                                      fx.Utils.showTips(`Reward claim failed.`);
                                  }
                              ...
                              console.log('skinDialog11, 商城看视频获取金币...AITA');
                              y.instance.getPlayerInfo().addCoins(this.getVideoReward());
                              fx.Effect.getPropEffect(this.btn_getcoins, this.box_Coins);
                              fx.Utils.showTips(`+ ${this.getVideoReward()} Coins`);
                          }
                          ...
                          if (t && this.curTouchBoxVideo && this.curTouchBoxVideo.$_GID === e.gid) {
                              console.log('skinDialog11, 恭喜获得碎片激励视频...AITA');
                              if (AitaBridge.inApp) {
                                  fx.Utils.showTips("Ad is loading... Please wait.");
                                  AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                                      if (Number(res) === 1) {
                                          fx.Utils.showTips('Nice! You received a Fragment!');
                                          x.inst.unlockSkin(t.id);
      ...
      updateUnlockUI(e, t, i) {
          ...
          if (I.VIDEO == s.way || m) {
              ...
              let e = 0;
              i && (e = i.videoCnt);
              let s = `AD Unlock${e}/${t.unlock.videoNum}`;
          ...
          if (!x.inst.isOwnSkin(i.id) && !x.inst.isTryTimeSkin(i.id)) {
              if (i.unlock.way === I.COINS) {
                  ...
                  I.SIGNIN == i.unlock.way
                      ...
                      : fx.Utils.showTips('This skin is locked!');
              return;
          }
      ...
      buySkin(e, t, i, s) {
          ...
          n &&
              ...
              (y.instance.getPlayerInfo().coins < n
                  ? fx.Utils.showTips('Not enough coins')
                  : (x.inst.unlockSkin(t),
                      y.instance.getPlayerInfo().useCoins(n),
                      this.updateUnlockUI(e, i, s),
                      fx.Utils.showTips('Skin unlocked successfully!')));
      }
      onChangeSkin() {
      ...
      onClickRandom() {
          if (!y.instance.coinsEnough(this.getRandomCost()))
              return void fx.Utils.showTips('Not enough coins');
          ...
          x.inst.unlockSkin(i), fx.Utils.showTips('Successfully unlocked!'), this.updateUI(), this.list_list.refresh();
      }
      getRandomCost() {
                      ...
                      AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                          if (Number(res) === 1) {
                              e.code === fx.SdkCode.REWARD_GAIN_SUCCESS &&
                                  this.btn_videoGet['gid'] === e.$GID && (console.log('BoxRewardDialog, 游戏结束奖励...AITA,激励广告'),
                                  this.getReward(0),
                                  this.getReward(1),
                                  this.getReward(2),
                                  fx.Utils.showTips('Reward received! Try your luck again!'),
                                  this.getBox(),
                                  this.initCard(0),
                                  this.getAgain(),
  ...
  class GameFailedDialog extends ui.scenes.GameFailedDialogUI {
      ...
      closeSelf(e) {
          console.log('GameFailedDialog11, 重新开始游戏,激励广告...AITA');
          if (AitaBridge.inApp) {
              fx.Utils.showTips("Ad is loading... Please wait.");
              AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                  if (Number(res) === 1) {
  ...
  class LotteryMultDialog extends ui.scenes.luckyDraw.LotteryMultDialogUI {
      ...
      getReward(e = 1) {
          ...
          fx.Effect.getPropEffect(this.label_coins, this.box_Coins, Laya.Handler.create(this, () => {
              console.log('LotteryMultDialog, 游戏结束开宝箱5倍奖励,激励广告...AITA');
              if (AitaBridge.inApp) {
                  fx.Utils.showTips("Ad is loading... Please wait.");
                  AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                      if (Number(res) === 1) {
      ...
      onEnter() {
          this.img_closeBtn.on(Laya.Event.CLICK, this, function () {
              ...
              this.getLotteryNumBtn.on(Laya.Event.MOUSE_DOWN, this, () => {
                  console.log('LotteryDialog, 游戏结束开宝箱奖励...AITA,激励广告');
                  if (AitaBridge.inApp) {
                      fx.Utils.showTips("Ad is loading... Please wait.");
                      AitaBridge.showRewardAdV2('ca-app-pub-8482820298032478/3301066669', (res) => {
                          if (Number(res) === 1) {
